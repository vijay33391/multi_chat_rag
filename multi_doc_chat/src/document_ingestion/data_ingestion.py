...
class ChatIngestor:
    def __init__( self,
        ...
        try:
            ...
            self.temp_dir = self._resolve_dir(self.temp_base) # e.g. "data/abc123" added subdir for session
    ...
    def _resolve_dir(self, base: Path):
        """ adding subdir for each session if enabled """
        if self.use_session:
    ...
    def built_retriver( self,
        ...
        *,
        ...
        ''' Build or update FAISS index from uploaded files and return a retriever '''
        try:
...
'''
 Creating a FAISS index (first time)
 Loading an existing FAISS index (next times)
 Avoiding duplicate document ingestion (idempotent ingestion)
 Keeping metadata about what was already added
'''
class FaissManager:
    ...
    def load_or_create(self,texts:Optional[List[str]]=None, metadatas: Optional[List[dict]] = None):
        ''' Load existing FAISS index or create a new one from texts and metadatas '''
        if self._exists():
